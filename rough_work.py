...
############################################################
        ...
        def answer():
            ans = check1.get() + check2.get() + check3.get() + check4.get()
            # print(ans)
            # anss.append(ans)
            # anss.insert(0,ans)

            l1 = Label(frame1, text=(f"Right answer = {ans//6} Wrong answer = {4-ans//6}"),font="lucida 22 bold",bg="green",fg="white")
            l1.grid()
            attempt = 1
        ...
        Button(frame, text="Next->", font="lucida 15 bold", bg="#7fff00", fg="white", command=next_page).grid(row=3,column=2)
        # answer()
